...
InsuLink uses different APIs to get all informations about food and storing user data.
	...
	\subsubsection{Helper class}
		...
		Following a list of the api class methods:

		\begin{lstlisting}[language=Python, caption=Api Helper]
			class Api{
				getSportCalories(userInput);
				getFoodList(userInput);
				getFoodListBarCode(barrcode);
				getIngredientDetails(foodName)
	...
	\subsection{Firebase}
		...
		\begin{lstlisting}[language=Python, caption=Firebase Helper]
			class FirebaseQuery{
				this.users = firestore().collection(userTable);
				getUser(uid);
				editUser(uid,data);
				getGlicemy(uid,date);
				addGlicemy(uid,date,glicemy);
				getDiary(uid,date);
				saveDiary(uid,date,diary);
			}
		\end{lstlisting}
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\vspace{10.5cm}
To manage app states we opted for Redux library \cite{Redux} affiancated with Redux Thunk Middleware \cite{Thunk}
\begin{center}
...
The main two reducers of the app are:
	...
	\begin{lstlisting}[language=Python, caption=UserReducer]
		const initialState = {
...
\begin{docCommand}{macroTracker}{}
	...
	\begin{lstlisting}[language=Python, caption=MacroReducer]
...
To make tests the mocking of firebase/ApiHelper and other components was neded.
A mocked version of redux was provided to a "<Provider>" components to emulate state changing behaviour.
\begin{center} 
	\begin{tabular}{ |p{4cm}|p{5cm}|  }
		\hline
		Food & 
		1. Snapshot \newline    
		2. Snapshot for deletable version \newline
		3. OnPress navigation\newline 
		\\
		\hline
		Food Details &
		1. Snapshot \newline  
		...
		3. Edit Food \newline   
		4. Delete Food \newline 
		(Checked both redux state and navigation)
		\\
		\hline
		Meal Diary &
		1. Snapshot \newline  
		2. Click on meal(navigation) \newline
		3. Click on AddFood/Sport \newline    
		(Checked navigation)
		\\
		\hline
		Home & 
		1. Snapshot \newline    
		2. Click on the 4 Buttons \newline
		(check navigation to correct page)
		\\
		\hline
		Login & 
		1. Snapshot \newline    
		2. Click on login button \newline
		3. Check if "notLogged" status is redirected to Login Page \newline
		(check redux state changed)
		\\
		\hline
		Registration & 
		1. Snapshot \newline    
		2. Click on Reg button \newline
		3. Check on Errors wrong inputs \newline
		\\
		\hline
	\end{tabular}
	\end{center}
...
\begin{thebibliography}{17}
	\vspace{10.5cm}
		...
		\bibitem{testRenderer} Source for React Test Renderer\\
				\url{https://it.reactjs.org/docs/test-renderer.html}
			\end{thebibliography}
			\subsection*{Credits}
			Special thanks to Matteo Ciancio who designed the app logo. For any 
			business contact you can reach him at: matteociancio99@gmail.com
